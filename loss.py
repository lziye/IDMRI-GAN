...
def seg_loss(output, target, miu_bce=1., miu_dice=1., eps=1e-8, smooth=1e-5, name='seg_loss'):
    with tf.name_scope(name):
        label = 1
        bce = binary_cross_entropy(output, target, label, eps=eps)
        d_coe = tf.reduce_mean(label * dice_coe(output, target, smooth=smooth, axis=[1,2,3]))
        return miu_bce * bce + miu_dice * (1. - d_coe)


def infer_g_init_train(x, y_):
    miu_bce = 1.
    miu_dice = 1.
